...
- **Extracts.** Given a photo of an invoice or receipt, it reads the supplier, document
  ...
  (`AiService.messages` in `apps/api/src/intelligence/ai.service.ts`, called from
...
- **Drafts.** Nothing implemented yet in this build calls the model to draft prose (the
...
- **Never calculates.** Every amount that reaches the ledger is computed in SQL or in the
  ...
  a total looks wrong.
- **Never posts.** There is no code path from `AiService` to `journal_entries` or
...
- `AiService` is the only class in the codebase that imports the `openai` package. Search
...
- If `OPENAI_API_KEY` is not set, `AiService.isConfigured` is `false` and every
...
`OPENAI_API_KEY is not configured...` and the document sits at `UPLOADED`. A user can
